...
Card Descriptions:
  ...
  Keywords are special words that are explicitly defined in the code. It's automatically colored yellow and display a tooltip when hovered over. They must be capitalized in the translation file, and must be separated from the rest of the text with spaces, even in languages that normally do not use spaces. Otherwise, they will not be recognized.
  ...
  Below are the special symbols used in card descriptions. Do not alter them.
    ...
    *: Colored word. Game automatically color keywords, so this is mostly for created cards.
    NL: Manual line break. These are optional and you can add/remove/relocate them however you want. But practically, you only need them for visual separation between unrelated card effects. Game automatically inserts line breaks to fit the text in the text box.
